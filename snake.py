...
def prob2bound_slicewise(i, prob_map_b, thres=0.7, ks=9):
    ...
    ahd_thres = 10.0
    lses = []
    pred = np.argmax(prob_map_b, axis=0) # for calculating ahd between pred and bound
    ...
    for bound_inx in range(1, n_channel):  # inner and outer bound
        ...
        for inx in range(iter_max):
            ...
            if len(contours) > 0: # early stop before finishing all loops
                # print("break at the {}th iteration".format(inx))
                break
    ...
    # ahd_pred_reg = slicewise_ahd(pred, reg, 3)
    # print("{} : AHD -- {:.4f}".format(i, ahd_pred_reg))
    # if ahd_pred_reg > ahd_thres:
    #     raise Exception("Slice {}: gap between prediction and constraint boundary is higher than {:.4f}".format(i, ahd_thres))

    return reg


def probmap2bound(prob_map, n_workers=32, thres=0.7, kernel_size=9):
    """ calculate constrained boundary from probmap as post-processing
    ...
    args = []
    for b in range(batch_size):
        # print("Processing {}th batch".format(b+1))
        args.append((b, prob_map_np[b], thres, kernel_size))

    pool = Pool(processes=n_workers)
    bounds = pool.starmap(prob2bound_slicewise, args)
    pool.close()
...
def prob2snake_slicewise_debug(i, prob_map_b, label_b, thres=0.7, do_plot=True):
    ...
    for bound_inx in range(1, n_channel):  # inner and outer bound
        ...
        prob_map_bb = prob_map_b[bound_inx]
        pred_filter = (prob_map_bb >= thres).astype(np.uint8)
        # image_open = cv2.morphologyEx(pred_filter, cv2.MORPH_OPEN, kernel_open)
        # gray = mask2gray(pred_filter)
        ...
        # image_close = cv2.morphologyEx(pred_filter, cv2.MORPH_CLOSE, kernel, iterations=7)

        # image_close = cv2.dilate(pred_filter, kernel, iterations=5)
        # image_close = cv2.erode(image_close, kernel, iterations=5)

        for inx in range(30):
            image_close = cv2.morphologyEx(pred_filter, cv2.MORPH_CLOSE, kernel_close, iterations=inx)
            # print(image_close.dtype, image_close.max(), image_close.min())
            # image_close = image_new.copy()
            # if i == 0:
            #     plt.imshow(image_close)
            #     plt.savefig("./Snake_results/{}.png".format(inx))
            _, contours, _ = cv2.findContours(image_close, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            contours = [contour for contour in contours
                        if len(contour) > 4 and cv2.contourArea(contour) / len(contour) >= 4.0]
            ...
            if len(contours) > 0:  # a closed contour can be obtained by cv2.findContours
                print("break at {}-th iteration".format(inx))
                break
    ...
    if do_plot:
        ...
        bound_pred = lslist2bound(lses)
        overlap = bound_pred.copy()
        overlap[label_b != 0] = 4


        ax[1, 3].imshow(mask2rgb(overlap))
        ax[1, 3].set_axis_off()
        # ax[1, 3].contour(lses[1], [0.5], colors='b')
        ax[1, 3].set_title("{} overlap".format(methods[fcs[1]]), fontsize=6)
    ...
    print("AHD: Pred - {:.4f}, Reg - {:.4f} Pred&Reg diff - {:.4f}, HD95: Pred - {:.4f}, Reg - {:.4f}".format(
...
def test_probamap2snake():
    ...
    data_dir = "./BoundDetection/Experiment9/HybridResUNet_int15_ds1_baseline_new/S218901d56_S20b24ded6ab723_20160803/" \
               "I12/data.pkl"
    ...
    # bounds_cuda = probmap2bound(prob_maps_cuda, n_workers=32, thres=0.7, kernel_size=9)
